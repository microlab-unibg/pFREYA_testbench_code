...
vthrp = (int)(sys.argv[2])
vthrn = (int)(sys.argv[3])
thrgenRef = (int)(sys.argv[4])
currentStep = (float)(sys.argv[5])
currentStep = abs(currentStep)
timeSleep = (int)(sys.argv[6])
...
start, end = filtro(percScatti)
fCurrent = current[start:end]
fCharge = charge[start:end]
fAvg = avg[start:end]
fPercScatti = percScatti[start:end]
fSdev = sdev[start:end]
fMax = max[start:end]
fMin = min[start:end]
...
results['Current level'] = fCurrent
results['Charge'] = fCharge
results['Mean'] = fAvg
results['% SOT'] = fPercScatti
results['Sdev'] = fSdev
results['Max #SOT'] = fMax
results['Min #SOT'] = fMin
...
errorFunct(fCharge, y, "Charge [ke-]", ylabel, title, timestampStr+"_charge")