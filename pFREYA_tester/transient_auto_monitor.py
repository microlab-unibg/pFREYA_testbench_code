...
for item in config_bits_list:
    ...
    config.config(channel='monitor', lemo='none', n_steps=8, cfg_bits=item, cfg_inst=True, active_probes=False)
    ...
    for i, cl in enumerate(config.current_lev):
        ...
        data['Amplitude (V)'] = data['Amplitude (V)'] / gain_mon
    ...
    df.to_csv(f'G:/Shared drives/FALCON/measures/new/transient/monitor/{channel_name}_{config.config_bits_str}_nominal_{lemo_name}_{datetime_str}.tsv', sep='\t')
    ...
    path=(f'G:/Shared drives/FALCON/measures/new/transient/monitor/{channel_name}_{config.config_bits_str}_nominal_{lemo_name}_{datetime_str}.tsv')
    ...
    config.config(channel='monitor',n_steps=8,cfg_bits=item,cfg_inst=False,lemo=False)
    ...
    plt.savefig(f'G:/Shared drives/FALCON/measures/new/transient/monitor/{channel_name}_{config.config_bits_str}_nominal_{lemo_name}_{datetime_str}.pdf',dpi=300)
