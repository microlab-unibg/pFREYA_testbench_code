...
# Funzioni specifiche che usano get_csa_bits 
# Funzioni basate su csa_bits 
def get_csa_bits(csa_bits): 
    match csa_bits: 
        case [0, 1]: 
            return {'photon_energy': 9, 'offset_charge': 8.5e-15, 'min_current': 0.07e-6, 'max_current': 1.55e-6, 'corr_fact': 1} 
        case [0, 0]:
            return {'photon_energy': 25, 'offset_charge': 8.5e-15, 'min_current': 0.07e-6, 'max_current': 3.8e-6, 'corr_fact': 1} 
        case [1, 0]: 
            return {'photon_energy': 18, 'offset_charge': 8.5e-15, 'min_current': 0.07e-6, 'max_current': 3.3e-6, 'corr_fact': 1} 
        case [1, 1]: 
            return {'photon_energy': 5, 'offset_charge': 0, 'min_current': 0.065e-6, 'max_current': 0.83e-6, 'corr_fact': 1} 
        case _: 
            raise ValueError("Configurazione csa_bits non valida")
def get_min_current(csa_bits): 
    return get_csa_bits(csa_bits)['min_current'] 
def get_max_current(csa_bits): 
    return get_csa_bits(csa_bits)['max_current'] 
def get_photon_energy(csa_bits): 
    return get_csa_bits(csa_bits)['photon_energy'] 
def get_offset_charge(csa_bits): 
    return get_csa_bits(csa_bits)['offset_charge'] 
def get_corr_fact(csa_bits): 
    return get_csa_bits(csa_bits)['corr_fact']
def auto_current_level_csa(csa_bits, n_steps): 
    current_levels = [] 
    min_current = get_min_current(csa_bits) 
    max_current = get_max_current(csa_bits) 
    current_lev = -1 * np.linspace(min_current, max_current, n_steps) 
    current_levels.append(current_lev) 
    return current_levels 
def auto_iinj_int(csa_bits, current_levels, T, t_r, N_pulses): 
    iinj_int_results = [] 
    for item in current_levels: iinj_int = item * (T/2 - t_r) * N_pulses + get_offset_charge(csa_bits) 
    iinj_int_results.append(iinj_int) 
    return iinj_int_results 
def auto_eq_ph(csa_bits, iinj_int_results, conv_kev_c): 
    eq_ph_results = [] 
    for item in iinj_int_results: eq_ph = -1 * item * get_corr_fact(csa_bits) * conv_kev_c / get_photon_energy(csa_bits) 
    eq_ph_results.append(eq_ph) 
    return eq_ph_results
